class ParsingController < ApplicationController
  ...
  def findParsing()
    ...
    when "http://shoppingw.naver.com"
            naver_shoppingwindow(breakParameter(params))

    when "http://www.lotteimall.com"
            lotteimall(breakParameter(params))

    when "http://www.oclock.co.kr"
            cjmall(breakParameter(params))

    when "http://www.jogunshop.com"
            jogunshop(breakParameter(params))

    when "http://store.melon.com"
            melon(breakParameter(params))

    when "http://www.hyundaihmall.com"
            hyundaihmall(breakParameter(params))
  ...
  def jogunshop(url)
    doc = Nokogiri::HTML(open(url))
    title = doc.css("title").children.tex
